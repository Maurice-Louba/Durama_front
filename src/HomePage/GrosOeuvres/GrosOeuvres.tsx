...
const GrosOeuvres = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await axiosInstance.get("quatres-gros-categorie/gros-oeuvres/");
  ...
  return (
    <div className="bg-white py-12 sm:py-20">
      <div className="max-w-[1400px] mx-auto px-4">
        {/* En-tête amélioré */}
        <div className="text-center flex justify-between mb-12 sm:mb-16">

            <div className="flex items-center justify-center">
                <FaHardHat size={24} className="text-black" />
          <p className="text-sm ml-4 font-bold tracking-wider text-gray-600">GROS OEUVRES</p>
          </div>
            <button onClick={()=>navigate('/Boutique')} className="px-6 py-2 bg-black text-white text-sm sm:text-base font-semibold rounded-md hover:bg-gray-800 transition">
            Voir tout
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
          {quatreProduits.map((prod, index) => (
            <div
              key={index}
              className="bg-white  border- rounded-xl  border border-[#b5e955] overflow-hidden hover:shadow-lg transition-all duration-300 group"
            >
              {/* Image avec badges */}
